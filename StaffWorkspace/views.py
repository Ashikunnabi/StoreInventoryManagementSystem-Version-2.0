...
def input_form(request, catagory):
    ...
    items = Item.objects.filter(catagory=catagory.id).order_by(
        'name')  # Filtering item name from items using catagory.id
    vendors = Vendor.objects.filter(catagory=catagory)  # Collecting all vendors
    ...
    context = {
        ...
        "catagory": _catagory,
        "category_for_dropdown": Catagory.objects.all(),  # for dropdown getting all the categories
    }

    if not request.user.is_authenticated:
        return render(request, 'StaffWorkspace/loginPage.html')
    else:
        # Checking the user has permission to edit or not
        try:
            if request.user.groups.all()[0].name == "view":
                return redirect('report', catagory)
            else:
...
def input_form_submit(request, catagory):
    ...
    if request.method == "POST":  # Checking the submit form request is post or not
        ...
        added_by = request.user.username  # Getting the information of logged in user
        ...
        if purchase == '0' and issued == '0':  # If javascript is not available then this one will arise
            ...
            # Collecting the information of a row/object from different model/table.
    ...
    if catagory == 'None':
        ...
        if value1 is not None:
            ...
            report = Report.objects.filter(date__year=datetime.now().year,
                                           ...
                                           catagory=catagory.id)  # Only current months report
        report_search_suggest = Item.objects.filter(catagory=catagory.id)  # Auto suggest field for search option

    # Sending all needed values to web page via context
    context = {
        "date": datetime.now(),
        "report": report,
        "summary_of_the_day": summary_of_the_day(report),
...
def summary_of_the_day(report):
    purchase_item, issued_item = 0, 0
    total_change = []
    _report = report
    ...
    for item in items:
        for r in _report:
            if item == r.item_name:
                # counting purchased item numbe
                if r.issued == 0:
                    purchase_item = purchase_item + r.purchase
                # counting issued item numbe                    
                if r.purchase == 0:
                    issued_item = issued_item + r.issued
                    
        # All changes stored in total_change list to supply in template
        if purchase_item == 0 and issued_item == 0:
            pass
        else:
            total_change.append(
                [item.item_no, item.name, purchase_item, issued_item])
            purchase_item, issued_item = 0, 0
...
def report_monthly(request, catagory=None, value=None):
    ...
    expenditure_month = 0
    total_change = []  # Tracking the change for individual changes
    ...
    if catagory == 'None':
        if value1 is not None:
            ...
            for item_detail in item_all:  # All items
                for report in reports:  # Report of searched month
                    if str(item_detail.name) == str(
                            report.item_name):  # If there is any report on that month then continue
                        previous_balance = Report.objects.filter(date__year=year, date__month=month,
                                                                 item_name=item_detail.id).first().previous_balance
                ...
                if str(item_detail.name) not in report_item_in_this_month:  # Otherwise put last transition from last month
                    if int(month) == 1:
                        ...
                        report_not_in_this_month = Report.objects.filter(
                            date__range=[datetime(2000, 1, 1), datetime(int(year) - 1, 12, days_in_month)],
                            item_name=item_detail.id).last()
                    else:
                        days_in_month = calendar.monthrange(int(year), int(month) - 1)[1]  # Getting how many days in one month
                        report_not_in_this_month = Report.objects.filter(
                            date__range=[datetime(2000, 1, 1), datetime(int(year), int(month) - 1, days_in_month)],
                            item_name=item_detail.id).last()
                ...
                # calculating total expenditure for specific month                
                expenditure_month = expenditure_month + total_purchase_cost

                # All changes stored in total_change list to supply in template
                total_change.append(
                    [item_detail.item_no, item_detail.name, previous_balance, purchase_item, issued_item,
                     ending_balance, total_purchase_cost])
        ...
        if value1 is not None:
            ...
            reports = Report.objects.filter(date__year=year, date__month=month,
                                            catagory=catagory.id)  # Requesting for given month
                ...
                if str(item_detail.name) not in report_item_in_this_month:  # Otherwise put last transition from last month
                    if int(month) == 1:
                        ...
                        days_in_month = calendar.monthrange(int(year), int(month) - 1)[
                            1]  # Getting how many days in one month
                        report_not_in_this_month = Report.objects.filter(
                            date__range=[datetime(2000, 1, 1), datetime(int(year), int(month) - 1, days_in_month)],
                            item_name=item_detail.id).last()
                    try:  # If there is any transition then continue
...
def graph(request):
    ...
    reports = Report.objects.filter(date__year=datetime.now().year,
                                    date__month=datetime.now().month)  # Only current months report
    context = {
